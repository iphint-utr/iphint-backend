...
export class PaddleService {
  ...
  static async handleSubscriptionUpdated(data: any) {
    ...
    const existingSub = await Subscription.findOne({ paddleSubscriptionId: data.id })
      .select('userId pendingPlanTier pendingBillingCycle')
      .lean();
    ...
    const pendingTier = (existingSub as any)?.pendingPlanTier;
    const pendingCycle = (existingSub as any)?.pendingBillingCycle;
    const pendingApplied =
      !!pendingTier &&
      pendingTier === plan.tier &&
      pendingCycle === billingCycle;
    if (pendingApplied || newStatus === 'cancelled' || newStatus === 'paused') {
      unsetFields.pendingPlanTier = '';
      unsetFields.pendingBillingCycle = '';
      unsetFields.pendingChangeEffectiveAt = '';
    }
    ...
    if (subscription) {
      ...
      // Top up credits/alerts on renewal (status becomes active)
      if (newStatus === 'active') {
        await Promise.all([
      ...
      await createUserAlert(
        ...
        {
          title: `Your subscription has been updated to ${plan.name} (${billingCycle}).`,
          type: 'billing',
        },
      );
    }
